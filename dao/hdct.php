...
function hdct_insert( $soluong,$dongia, $mahd, $masp){
    $sql = "INSERT INTO chitiethoadon(soluong,dongia, mahd, masp) 
            VALUES (?,?,?,?)";
    pdo_execute($sql,$soluong,$dongia, $mahd, $masp);
}
function hdct_update( $tensp, $giamoi, $giamgia, $mota,$khuyenmai, $trangthai, $hot, $mathh, $macl, $maloai,$masp){
    $sql = "UPDATE sanpham 
            SET tensp=?,
                giamoi=?,
                giamgia=?,
...
function hdct_select_all(){
    $sql = "SELECT * from sanpham order by masp desc";
    return pdo_query($sql);
}


function hdct_select_by_masp($masp){
    $sql = "SELECT * from sanpham WHERE masp=?";
    return pdo_query_one($sql, $masp);
}
function hdct_select_by_mahdct_latest(){
    $sql = "SELECT * from hoadon order by mahd desc limit 0,1";
    return pdo_query_one($sql);
}
/**
 * Truy vấn theo giới tính */
function hdct_select_by_nam(){
    $sql = "SELECT * from sanpham WHERE gioitinh=1";
    return pdo_query($sql);
}
function hdct_select_by_nu(){
    $sql = "SELECT * from sanpham WHERE gioitinh=0";
    return pdo_query($sql);
}
function hdct_select_by_capdoi(){
...
function hdct_select_by_id_top8($maloai){
...
function hdct_select_by_maloai($maloai){
    $sql = "SELECT * from sanpham WHERE maloai=?";
    return pdo_query($sql,$maloai);
}
/** Truy vấn tìm kiếm */
function hdct_select_keyword($keyword){
