...
function hd_insert( $ghichu, $tonggia, $makh){
    $sql = "INSERT INTO hoadon(ghichu, tonggia, makh) 
            VALUES (?,?,?)";
    pdo_execute($sql,$ghichu, $tonggia, $makh);
}
function hd_update( $tensp, $giamoi, $giamgia, $mota,$khuyenmai, $trangthai, $hot, $mathh, $macl, $maloai,$masp){
    $sql = "UPDATE sanpham 
            SET tensp=?,
                giamoi=?,
                giamgia=?,
...
function hd_select_by_id_top8($maloai){
...
function hd_select_by_maloai($maloai){
    $sql = "SELECT * from sanpham WHERE maloai=?";
    return pdo_query($sql,$maloai);
}
/** Truy vấn tìm kiếm */
function hd_select_keyword($keyword){
